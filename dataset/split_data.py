...
def main():
    ...
    # start splitting
    sorted_pfams = sorted(pfam_to_cluster_dict.keys(), reverse=True,
                          key=lambda k: len([pfam_to_cluster_dict[k]]))
    current_split = {}
    for pfam in sorted_pfams:
        pfam_clusters = pfam_to_cluster_dict[pfam]
        ...
        missing_splits = [split_index for split_index in range(len(SPLIT)) if number_of_protein_split[split_index] == 0]
        ...
        sorted_clusters = sorted(pfam_clusters, key=lambda pfam_cluster: len(cluster_dict[pfam_cluster]), reverse=True)
    ...
    save_splits("dataset/subsets/complicated_subset/split.json", split_by_protein[0], split_by_protein[1], split_by_protein[2])
